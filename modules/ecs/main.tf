...
data "template_file" "db_create_task" {
  template = file("${path.module}/tasks/db_create_task_definition.json")
...
resource "aws_ecs_task_definition" "db_create" {
  family                   = "${var.environment}_db_create"
...
resource "aws_alb" "alb_terraform_ecs" {
  name            = "${var.environment}-alb-terraform_ecs"
...
resource "aws_alb_listener_rule" "terraform_ecs_listener_rule" {
  listener_arn = aws_alb_listener.terraform_ecs.arn
  priority     = 100
